...
def incorporate(img_path, compo_path, text_path, output_root, params, show=False, wait_key=0):
    ...
    draw_bounding_box_class(org, compos, show=show, name='ip', wait_key=wait_key)
    draw_bounding_box(org, texts, show=show, name='ocr', wait_key=wait_key)

    compos_merged = reclassify_text_by_ocr(org, compos, texts)
    ...
    # merge words as line
    compos_merged = merge_intersected_compos(org, compos_merged, max_gap=(params['max-word-inline-gap'], 0), merge_class='Text')
    draw_bounding_box_class(org, compos_merged, name='merged line', show=show, wait_key=wait_key)
    # merge lines as paragraph
    compos_merged = merge_intersected_compos(org, compos_merged, max_gap=(0, params['max-line-gap']), merge_class='Text')
    ...
    board = draw_bounding_box_class(org, compos_merged, name='merged paragraph', is_return=True, show=show, wait_key=wait_key)

    # draw_bounding_box_non_text(org_resize, compos_merged, org_shape=org.shape, show=show)
    compos_json = save_corners_json(output_root, background, compos_merged, org.shape)
    dissemble_clip_img_fill(pjoin(output_root, 'clips'), org, compos_json)
