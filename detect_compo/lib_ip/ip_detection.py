...
def rm_lines(binary,
    ...
    if show:
        cv2.imshow('binary (no-line)', binary)
...
def compo_filter(compos, min_area):
    ...
    for compo in compos:
        ...
        #     continue
        if ratio_w > 40 or compo.height < 8:
            continue
        compos_new.append(compo)
    return compos_new


def filter_url_box(compos, min_height):
    url_boxes = []
    for compo in compos:
        if compo.height < min_height:
            continue
        ratio_h = compo.width / compo.height
        if ratio_h < 5:
            continue
        url_boxes.append(compo)
    return url_boxes
        ...
        for j in range(i % 2, column, step_v):
            if binary[i, j] == 255 and mask[i, j] == 0:
                ...
                if component.width <= 2 or component.height <= 2:
                    continue
                if component.width >= column - 2 or component.height >= row - 2:
                    continue
